...
def load_inference_bundle(model_name: str, checkpoint: str, data_root: str):
    ...
    device = get_device()
    model = build_model(
        model_name, num_classes=len(classes), pretrained=False, freeze_backbone=False
    )
...
def main():
    ...
    model, classes, device, transform = load_inference_bundle(
        args.model_name, args.checkpoint, args.data_root
    )
