...
def save_history_plot(
    history: list[dict[str, Any]], out_path: PathLike, title_prefix: str
) -> None:
    if not history:
        return
...
def top_confusions(
    y_true: list[int], y_pred: list[int], class_names: list[str], top_k: int = 15
...
def write_top_confusions_csv(
    rows: list[tuple[str, str, int]], out_path: PathLike
) -> None:
    out_path = Path(out_path)
    out_path.parent.mkdir(parents=True, exist_ok=True)
