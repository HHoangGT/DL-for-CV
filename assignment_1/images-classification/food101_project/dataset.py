...
def resolve_food101_paths(data_root: PathLike = DATA_ROOT) -> Food101Paths:
    ...
    for base in candidates:
        ...
        if (
            images_dir.exists()
            and train_txt.exists()
            and test_txt.exists()
            and classes_txt.exists()
        ):
            return Food101Paths(
                base, images_dir, meta_dir, train_txt, test_txt, classes_txt
            )
...
def load_classes(paths: Food101Paths) -> list[str]:
    with open(paths.classes_txt, encoding="utf-8") as f:
        return [line.strip() for line in f if line.strip()]


def _load_split(
    txt_path: Path, images_dir: Path, class_to_idx: dict[str, int]
) -> pd.DataFrame:
    rows = []
...
def build_splits(
    data_root: PathLike = DATA_ROOT, val_ratio: float = VAL_RATIO, seed: int = SEED
):
...
class Food101CustomDataset(Dataset):
    def __init__(
        self, dataframe: pd.DataFrame, transform=None, return_path: bool = False
    ):
...
def create_dataloaders(
    ...
    classes, class_to_idx, train_df, val_df, test_df = build_splits(
        data_root=data_root, val_ratio=val_ratio, seed=seed
    )
    train_transform = (
        get_train_transform(img_size)
        if use_augmentation
        else get_plain_train_transform(img_size)
    )
