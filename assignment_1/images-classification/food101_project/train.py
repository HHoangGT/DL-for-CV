...
def parse_args():
    ...
    parser.add_argument(
        "--label_smoothing", type=float, default=DEFAULT_LABEL_SMOOTHING
    )
    ...
    unknown = sorted({m for m in args.models if m not in SUPPORTED_MODELS})
    if unknown:
        raise ValueError(
            f"Model không hỗ trợ: {unknown}. Các model hợp lệ: {SUPPORTED_MODELS}"
        )
    return args
...
def main():
    ...
    for model_name in args.models:
        ...
        save_history_plot(
            train_out["history"],
            run_figure_dir / f"{model_name}_history.png",
            model_name,
        )
        ...
        summary["training_time_hms"] = format_seconds(
            train_out["training_time_seconds"]
        )
